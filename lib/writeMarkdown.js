...
let globalLines = []
let topPath;
function writeWord(obj, staticList, stateList, refList, toppath,globalComments) {
  globalLines = []
  topPath = toppath
  let word = '';
  console.log(stateList);
  word += `## 组件中的function管理\n`
  word += appendOtherFunc(obj, 0, '', stateList,staticList)
  fs.writeFileSync(path.resolve(topPath, '../apply.md'), word)
}

function appendOtherFunc(obj, lowNum, parentName, stateList, staticList) {
  let word = ''
  for(let propName in obj) {
    if(staticList.includes(propName)) {
      continue;
    }
    if(obj[propName]?.params?.[0] === 'props') {
      if(obj[propName].commentLines && obj[propName].commentLines.length !== 0){
        const propLine = obj[propName].commentLines[0].split('*');
        for(let i=0; i<propLine.length; i++) {
          const index = propLine[i].indexOf('props.');
          if(index > -1) {
            const indexEnd = propLine[i].indexOf('\n');
            propsLines.push(propLine[i].slice(index, indexEnd))
          }
        }
      }
    }
    ...
    if(propName !== 'state') {
      ...
      if(func.applyList) {
        ...
        if(func && func.applyList) {
          func.applyList.forEach((apply,index) => {
            word += ` (${index + 1}) **${apply}**: ` + propsLines.find(item => item.includes(apply)) + '\n'
          })
        }
      }
      ...
      if(func && func.ownFuncList) {
        func.ownFuncList.forEach((ownFunc,index) => {
          let message = Object.keys(stateList).find(key => key === ownFunc);
          if(message) {
            message = stateList[message]
          }else{
            const item = globalLines.find(item => item.includes(ownFunc));
            if(item) {
              message = item.replace('\n','')
            }
          }
          word += ` (${index + 1}) ${ownFunc}${message ? ' => ' + message : ''}\n`
        })
      }
