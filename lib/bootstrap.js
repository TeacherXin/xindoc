  ...
  traverse(AST, {
    enter(itemNode) {
      ...
      if(alowSaveFuncList.includes(itemNode.node.type)) {
        ...
        //计算方法的参数
        let paramsList
        if(itemNode.node.type === 'FunctionDeclaration') {
          paramsList = itemNode.node?.params || [];
          params = paramsList.map(item => {
            return item.name
          })
        }else{
          paramsList = arrowFunctionExpression?.params || [];
          params = paramsList.map(item => {
            return item.name
          })
        }
        ...
        if(leveIndex > -1) {
          funApplyFunList[funcName].level = commentLines[0][leveIndex + 7];
        }else {
          funApplyFunList[funcName].level = 0;
        }
...
function appendOtherFunApply(otherFuncList, importModule, childNowPath) {
  Object.keys(otherFuncList).forEach(func => {
    ...
    // console.log(funApplyFunList[func]);
