  ...
  if(funcName === 'state') {
    itemNode.traverse({
      ObjectProperty(objectProperty) {
        ...
        stateList.classState.push(`${objectProperty.node.key.name}: ${leadingComments?.toString() || '未标明'}`);
      }
    })
  }else{
    itemNode.traverse({
      VariableDeclaration(variableDeclaration) {
        ...
        if(variableDeclarator?.id?.elements?.length === 2) {
          if(variableDeclarator?.init?.callee?.name === 'useState') {
            ...
            stateList[variableDeclarator.id.elements[1].name] = `${variableDeclarator.id.elements[0].name}: ${leadingComments?.toString() || '未标明'}`;
...
function dealCallback(itemNode, importModule, otherFuncList, publicFunclist, ownFuncList) {
  itemNode.traverse({
    CallExpression(callExpression) {
      ...
      const comline = callExpression.node.loc.start.line;
      if(fileString[comline - 2].includes('@discript')) {
        callName += fileString[comline - 2].replace('@discript','')
      };
  ...
  traverse(AST, {
    enter(itemNode) {
       ...
       */
        ...
        //获取async状态
        const asyncFlag = itemNode.node.declarations[0].init.async;
        params = dealParams(itemNode, params);
        dealState(funcName,itemNode, ownFuncList)
        dealCallback(itemNode, importModule, otherFuncList, publicFunclist, ownFuncList);
        dealComline(itemNode, commentLines)
...
function main() {
  ...
  writeWord(funApplyFunList, staticList,stateList, refList ,topPath,globalComments)
