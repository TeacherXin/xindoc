  ...
  if(funcName === 'state') {
    ...
    itemNode.traverse({
      VariableDeclaration(variableDeclaration) {
        ...
        if(variableDeclarator?.id?.elements?.length === 2 && variableDeclarator?.init?.callee?.name === 'useState') {
...
// 获取方法体
function dealFuncBody(funcName, itemNode, file) {
  let deepFlag = false;
  ...
  traverse(AST, {
    enter(itemNode) {
      ...
      if(alowSaveFuncList.includes(itemNode.node.type)) {
        ...
        const { deepFlag, closePkFlag} = dealFuncBody(funcName,itemNode, file);
        // 将所有的信息，综合处理，成为json的数据结构
        dealFuncAllrelation({funcName,funApplyFunList, ownFuncList, publicFunclist, newPath, funcbody, level, commentLines, params, otherFuncList,deepFlag, closePkFlag,asyncFlag})
        //找到外部方法的函数依赖关系
       ...
       */
        ...
        funcbody = dealFuncBody(funcName,itemNode, file).funcbody
        const { deepFlag, closePkFlag} = dealFuncBody(funcName,itemNode, file);
        dealFuncAllrelation({funcName,funApplyFunList, ownFuncList, publicFunclist, newPath, funcbody, level, commentLines, params, otherFuncList,deepFlag, closePkFlag})
        //找到外部方法的函数依赖关系
        appendOtherFunApply(funApplyFunList[funcName].otherFuncList, importModule, newPath)
      /**
       * 第四种情况：
       ...
       */
        ...
        dealComline(itemNode, commentLines)
        dealFuncAllrelation({funcName,funApplyFunList, ownFuncList, publicFunclist, newPath, funcbody, level, commentLines, params, otherFuncList})
        //找到外部方法的函数依赖关系
        appendOtherFunApply(funApplyFunList[funcName].otherFuncList, importModule, newPath);
        const applyList = itemNode.node.expression.arguments[1].elements.map(item => item.name);
...
function main() {
  ...
  writeWord(funApplyFunList, staticList,stateList, topPath)
