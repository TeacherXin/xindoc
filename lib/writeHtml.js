...
function writeHtml(funApplyFunList, staticList) {
  ...
  html += otherFuncItemList;
  html += getScript()

  console.log(funcItemList);
...
function getFuncItem(funcName, funcItem) {
  ...
  funcItemEjs += getFuncPath(funcItem.yydsFuncList)
...
function getFuncPath(funcList, parent) {
  ...
  if(Array.isArray(funcList)) {
    ...
    for(let propName in funcList) {
      ...
      otherFuncItemList += getFuncItem(propName, funcList[propName]);
    }
  }
  return result + '</ul>'
}
